...
function Index() {
  ...
  const chartData = useMemo(() => {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      days.push({ date: d.toLocaleDateString(undefined, { weekday: "short" }), total: dayTotal });
    }
    return days;
  }, [sales, closings]);
      ...
      <div className="space-y-6">
        <div className="flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Real-time overview of sales, stock, and alerts
            </p>
          </div>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <StatCard
            ...
            sub={`${alerts.filter((a) => a.urgent).length} urgent`}
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={chartData}>
                    ...
                    <Tooltip
                      ...
                      formatter={(v: number) => [`${v.toFixed(2)} ${CURRENCY}`, "Sales"]}
                    />
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              ...
              {alerts.slice(0, 5).map((a) => (
                <div
                  key={a.id}
                  className={`rounded-lg border p-3 text-sm ${a.urgent ? "border-destructive/40 bg-destructive/10" : "border-warning/40 bg-warning/10"}`}
                >
              ...
              {alerts.length > 5 && (
                <Button asChild variant="ghost" size="sm" className="w-full">
                  <Link to="/notifications">View all {alerts.length}</Link>
                </Button>
              )}
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-base">
                ...
                Low stock (&lt; {LOW_STOCK_THRESHOLD} boxes)
              </CardTitle>
            </CardHeader>
            <CardContent>
              {lowStock.length === 0 ? (
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-base">
                ...
                Near expiry (&lt; {NEAR_EXPIRY_DAYS} days)
              </CardTitle>
            </CardHeader>
            <CardContent>
              {nearExpiry.length === 0 ? (
    ...
    <Card className={accent ? "border-primary/40" : danger ? "border-destructive/40" : ""}>
      <CardContent className="p-5">
        <div className="flex items-center justify-between">
          ...
          <span
            className={`flex h-9 w-9 items-center justify-center rounded-lg ${accent ? "bg-primary/15 text-primary" : danger ? "bg-destructive/15 text-destructive" : "bg-muted text-foreground"}`}
          >
            {icon}
          </span>
        </div>
