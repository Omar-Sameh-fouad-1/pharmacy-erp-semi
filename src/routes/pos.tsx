...
function PosPage() {
  ...
  const addToCart = (med: Medicine) => {
    // 1. منع بيع المنتهي الصلاحية
    if (checkIsExpired(med.expiryDate)) {
    ...
    <AppShell>
        ...
        <div className="space-y-4">
          <Card className="border-primary/30">
            ...
            <CardContent className="space-y-3">
              <Input ref={inputRef} value={barcode} onChange={(e) => setBarcode(e.target.value)} onKeyDown={onBarcodeKey} placeholder="امسح الباركود هنا..." autoComplete="off" className="h-14 text-lg tracking-widest text-right" dir="ltr" />
              {scannerError && (
          ...
          <Card>
            <CardHeader><CardTitle className="text-base text-right">بحث يدوي (بديل)</CardTitle></CardHeader>
            <CardContent className="space-y-3">
              <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="ابحث بالاسم أو الباركود..." className="text-right" dir="rtl" />
              {filtered.length > 0 && (
                <ul className="divide-y divide-border rounded-md border border-border">
                  {filtered.map((m) => {
                    const isExpired = checkIsExpired(m.expiryDate);
                    return (
                    <li key={m.id} className={`flex items-center justify-between p-3 text-sm flex-row-reverse ${isExpired ? "bg-destructive/10 opacity-75" : ""}`}>
                      <div className="text-right">
                        <div className={`font-medium ${isExpired ? "text-destructive" : ""}`}>{m.name}</div>
                        <div className="text-xs text-muted-foreground">{m.barcode} · {m.quantity} علبة · {formatMoney(m.sellingPrice)}</div>
        ...
        <Card className="flex h-[calc(100vh-9rem)] flex-col">
          ...
          <CardContent className="flex flex-1 flex-col gap-3 overflow-hidden">
            <div className="flex-1 overflow-y-auto pl-1">
              {cart.length === 0 ? (
                <div className="flex h-full flex-col items-center justify-center text-center text-sm text-muted-foreground">
                  <ShoppingCart className="mb-2 h-8 w-8 opacity-50" /> السلة فارغة. امسح باركود للبدء.
                </div>
              ) : (
                  ...
                  {cart.map((l) => (
                    <li key={l.medicineId} className="rounded-lg border border-border bg-muted/30 p-3">
                      ...
                      <div className="mt-2 flex items-center justify-between flex-row-reverse">
                        ...
                        <Button size="sm" variant="ghost" className="text-destructive hover:bg-destructive/10" onClick={() => setCart((c) => c.filter((x) => x.medicineId !== l.medicineId))}><Trash2 className="h-3.5 w-3.5" /></Button>
                      </div>
                    </li>
                  ))}
                </ul>
            ...
            <div className="space-y-3 border-t border-border pt-3">
              ...
              <Button className="w-full" size="lg" disabled={cart.length === 0} onClick={checkout}>دفع {formatMoney(total)}</Button>
            </div>
          </CardContent>
        </Card>
      </div>

      <Dialog open={!!invoiceData} onOpenChange={(open) => { if (!open) { setInvoiceData(null); setTimeout(() => inputRef.current?.focus(), 100); } }}>
        <DialogContent className="sm:max-w-md text-right rtl" dir="rtl">
            ...
            <div className="space-y-2 mb-4">
              {invoiceData?.items.map((item, idx) => (
                <div key={idx} className="flex justify-between flex-row-reverse">
                  <span>{item.qty}x {item.medicineName}</span><span>{formatMoney(item.qty * item.unitPrice)}</span>
                </div>
              ))}
            </div>
            <div className="border-t border-dashed border-gray-300 pt-4 font-bold flex justify-between text-base flex-row-reverse">
              <span>الإجمالي</span><span>{invoiceData && formatMoney(invoiceData.total)}</span>
            </div>
          ...
          <DialogFooter className="print:hidden flex-row-reverse justify-between mt-4">
            ...
            <Button onClick={() => window.print()} className="gap-2 bg-blue-600 hover:bg-blue-700 text-white flex-row-reverse">طباعة الفاتورة <Printer className="w-4 h-4" /> </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!pendingRx} onOpenChange={(open) => !open && setPendingRx(null)}>
        <DialogContent className="text-right" dir="rtl">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 justify-end">مطلوب روشتة طبية <AlertTriangle className="h-5 w-5 text-warning" /></DialogTitle>
            <DialogDescription className="text-right">الدواء <strong>{pendingRx?.name}</strong> يتطلب وصفة طبية صالحة (روشتة). يرجى التحقق من الروشتة قبل إتمام عملية البيع.</DialogDescription>
          </DialogHeader>
