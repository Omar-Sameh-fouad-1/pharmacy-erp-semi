...
function logAudit(action: string, details?: string, severity: AuditLog["severity"] = "info") {
  ...
  setState((s) => ({
    ...s,
    auditLogs: [
      { id: uid("a_"), ts: new Date().toISOString(), actorId: cu?.id || "system", actorName: cu?.fullName || "النظام", action, details, severity },
      ...s.auditLogs,
    ].slice(0, 500),
  }));
}
...
export function deleteSupplier(id: string) {
  setState((st) => ({
    ...
    medicines: st.medicines.map((m) => ({
      ...m,
      supplierIds: m.supplierIds.filter(sid => sid !== id)
    })),
  }));
...
export function resetDemoData() {
  setState(() => seed());
  logAudit("system.reset", "تم تهيئة النظام بالكامل", "critical");
}
